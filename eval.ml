...
let rec variableSwap (x : Uml.var) (y : Uml.var) (e : Uml.exp) =
  match e with
  | Uml.Var z ->
      if z = x then Uml.Var y else if z = y then Uml.Var x else Uml.Var z
  | Uml.Lam (z, e) ->
      if z = x then Uml.Lam (y, variableSwap x y e)
      else if z = y then Uml.Lam (x, variableSwap x y e)
...
let rec substitute (e' : Uml.exp) (x : string) (e : Uml.exp) =
  ...
  | e', x, Uml.Lam (y, e) ->
      if x = y then Uml.Lam (y, e)
        ...
        Uml.Lam (z, substitute e' x (variableSwap y z e))
...
let rec stepv e : Uml.exp =
  ...
  | Uml.Lam (x, e) -> Uml.Lam (x, stepv e)
  | Uml.App (Uml.Var x, e2) -> Uml.App (Uml.Var x, stepv e2)
  | Uml.App (Uml.Lam (x, e1), e2) -> substitute e2 x e1
